...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        handler2 = new Handler(){
            @Override
            public void handleMessage(@NonNull Message msg) {
                //Log.d("mythread", "handleMessage: "+msg.arg1);
                if(msg.what==1){
                    textView.setText("progressBar진행률:"+msg.arg1+"%");
                    progressBar.incrementProgressBy(1);
                }
            }
        };
    ...
    긴 시간 실행하며 view를 변경하는 경우
    실행되는 동안 다른 작업을 할 수 없다.
    실행이 되는 동안 사용자의 이벤트가 발생하고 이벤트에 5초동안
    반응하지 않으면 안드로이드 OS는 어플리케이션을 강제 종료한다.
    ANR(Application Not Responding)
    오랫동안 처리해야하는 작업을 UI 쓰레드에 정의하면 안된다.
    별도의 작업 쓰레드를 정의하고 실행
    */
    ...
    // - 잠정적인 문제점을 갖고 있는 방법(UI의 변경은 UI 쓰레드에서만 작업)

...
public class ReturnDataFirstActivity extends AppCompatActivity implements OnClickListener
{
	...
	public void onClick(View v) {
		if (v.getId() == R.id.btnExit) {
			...
			intent.putExtra("code", "call1");
			intent.putExtra("data", "첫 번째 액티비티가 넘기는 메시지");
			startActivity(intent);
		} else if (v.getId() == R.id.call2) {
			//새로운 액티비티를 실행해서 작업을 와뇨한 후 되돌아오는 작업을 수행
			Intent intent = new Intent(this, ReturnDataSecondActivity.class);
			intent.putExtra("code", "call2");
			intent.putExtra("data", "두 번째 액티비티가 넘기는 메시지");
	...
	//onAcitvityResult를 오버라이딩해서 처리할 작업을 구현
